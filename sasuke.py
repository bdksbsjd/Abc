...
bot = telebot.TeleBot('7550194604:')

# Admin user IDs
admin_id = ["7272842335:AAFLwtA-8R_Vww7WJnRBsqmmWSwQjINVfL4"]
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            ...
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < COOLDOWN_TIME:
                response = "You Are On Cooldown. Please Wait 120 sec Before Running The /attack Command Again buy premium instant attack with zero sec SHADOW."
                bot.reply_to(message, response)
                return
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 360:
                response = "Error: Time interval must be less than 360 seconds."
            else:
